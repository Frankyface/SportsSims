...
**Brand:** ESSPN is the network (broadcaster); it presents competitions — the **Crown League** (Soccer), the **Bastion Championships** (Rugby), and the **SGA Tour** (Golf — the Simulated Golf Association). Use the specific competition name for a table/league, "ESSPN" for the network wordmark.
...
- **Multi-sport isolation:** each new sport gets its own modules and must NOT entangle the live soccer path. Rugby lives in `sim/rugby*`, `render/rugby*`, `ratings/rugbyTeams`, `ui/RugbyTab` (`RUGBY_SIM_VERSION 1`). **Golf** (the SGA Tour — first *individual* sport) lives in `sim/golf*`, `render/golf*` (incl. `golfBrand` + `golfCourseArt`), `ratings/golfers|golfCourses|golfStrength`, `league/golfSeason`, `content/golf*`, `ui/GolfTab` — its own `GOLF_SIM_VERSION` (currently **3** — the "a guy having a day" boost) + golden (`golfScoreCompat.test.ts`), a 20-course pool rotating 10-of-20 per season, and its own brand crest (runtime-loaded `public/logos/sga.png` + drawn fallback). Reuse shared types/spine (league/persistence/content, `prng`, `glicko2`, the WebCodecs export + audio mixer), don't fork the soccer sim.

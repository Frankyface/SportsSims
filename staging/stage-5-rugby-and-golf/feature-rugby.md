...
- [~] **Realism v1→v3 (operator feedback loop) — PARKED, not signed off.** Three passes on the render/`:pbp` layer: backward-pass geometry, kick hang time + kick onside law, an attacker onside clamp (no dot goal-side of the ball), a weighted phase menu (pods/sweeps/switches/offloads/chips), a defensive line + sweeper, a line-break funnel, ruck de-jolt, slower pace (~70-82s), vertical orientation kept. All rule-tests pass (136 green). **Operator verdict after v3: the rugby match still doesn't look right ("the rugby editor is not working").** Decision 2026-07-08: park rugby, move to the Golf sim. Plan history: `~/.claude/plans/luminous-dazzling-beacon.md`.
  - **When revisited:** the gap is look/feel, not correctness — so step back from constant-tuning and reconsider the 2D REPRESENTATION itself (orientation/camera, dot count & size, how phases are staged, or a different abstraction that reads on a phone). Eyeball the LIVE site first (an in-session preview-harness bug blocked screenshots — it failed identically on the unchanged soccer canvas, so it's environmental). Also finish the rugby season-zip download.

...
_Last updated: 2026-07-08 · **V1 shipped & LIVE**. Post-V1 in progress: the match engine is now **square-race-style continuous play** — ~55-69s clips, flowing passes, a storyline commentator, REAL rules (red-card send-offs that leave a team with 7 dots + a scoring handicap, corner kicks), goal confetti/shake, per-game season-pack folders with Instagram captions, and a drop-in cheer/boo/music audio system (sounds pending from the user). Rugby **identity layer built** ahead of the rugby sim. **Next: user's sound files, then the rugby match engine.**_
...
- **Drop-in audio system**: `src/assets/audio/` (`music-*`/`cheer-*`/`boo-*`) mixed into exports deterministically; procedural fallback until the user provides sounds (see `help.md`).
